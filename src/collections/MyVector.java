...
public class MyVector implements List {
    ...
    public MyVector(int capacity) {
        if (capacity < 0)   // 매개변수가 0보다 작다면 예외를 던집니다.
    ...
    public Object get(int index) {
        if(index < 0 || index >= size)  // 매개변수가 0보다 작거나, 길이보다 크다면 예외를 던짐짐
    ...
    // 객체를 매개변수로 받아 삭제하는 메서드
    public boolean remove(Object obj) {
        for(int i=0; i< size; i++) {
            if(obj.equals(data[i])) {   // 순차적으로 탐색하여 동일한 값이 존재한다면 삭제.
                remove(i);
                return true;
            }
        }
